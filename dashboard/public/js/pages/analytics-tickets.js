...
window['page-analytics-tickets'] = {
  ...
  async render(container) {
    ...
    container.innerHTML = `
      ...
      <div class="grid-2">
        <div class="chart-card"><div class="card-header"><div class="card-title">Tickets pro Tag</div></div><div style="height:220px"><canvas id="ta-daily-chart"></canvas></div></div>
        <div class="chart-card"><div class="card-header"><div class="card-title">Nach Kategorie</div></div><div style="height:220px"><canvas id="ta-cat-chart"></canvas></div></div>
      </div>
    `;
  ...
  async load() {
    try {
      ...
      document.getElementById('ta-stats').innerHTML = `
        <div class="stat-card accent"><div class="stat-label">Gesamt</div><div class="stat-value">${total}</div><div class="stat-sub">alle Tickets</div></div>
        <div class="stat-card warning"><div class="stat-label">Offen</div><div class="stat-value" style="color:var(--warning)">${open}</div><div class="stat-sub">aktiv</div></div>
        <div class="stat-card online"><div class="stat-label">Geschlossen</div><div class="stat-value" style="color:var(--online)">${closed}</div><div class="stat-sub">abgeschlossen</div></div>
