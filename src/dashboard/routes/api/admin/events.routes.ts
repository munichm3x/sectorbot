...
import { pushEventToDiscord, deleteEventFromDiscord } from '../../../../services/discordSync';

export function eventsAdminRouter(client: Client): Router {
  const router = Router();
  ...
  router.post('/', (req, res) => {
    try {
      ...
      pushEventToDiscord(client, created.id).catch(err =>
        logger.error('[sync] event push failed:', err)
      );
    } catch (err) {
  ...
  router.patch('/:id', (req, res) => {
    try {
      ...
      pushEventToDiscord(client, id).catch(err =>
        logger.error('[sync] event push failed:', err)
      );
    } catch (err) {
  ...
  router.delete('/:id', (req, res) => {
    try {
      ...
      // Capture discord_event_id BEFORE deleting from DB
      const discordEventId = (existing as { discord_event_id?: string | null }).discord_event_id;
      ...
      res.json({ success: ok });
      if (discordEventId) {
        deleteEventFromDiscord(client, discordEventId, user.guildId).catch(err =>
          logger.error('[sync] event delete from discord failed:', err)
        );
      }
    } catch (err) {
