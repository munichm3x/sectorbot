...
// GET /api/rules/:id
rulesAdminRouter.get('/:id', (req, res) => {
  try {
...
rulesAdminRouter.post('/reorder', (req, res) => {
  try {
...
rulesAdminRouter.patch('/:id', (req, res) => {
  try {
    const id = Number(req.params.id);
    const user = req.session.user!;
    if (!Number.isFinite(id)) {
...
// DELETE /api/rules/:id
rulesAdminRouter.delete('/:id', (req, res) => {
  try {
    const id = Number(req.params.id);
    const user = req.session.user!;
    if (!Number.isFinite(id)) {
