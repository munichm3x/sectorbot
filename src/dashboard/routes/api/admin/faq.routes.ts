...
faqAdminRouter.post('/reorder', (req, res) => {
  try {
...
faqAdminRouter.patch('/:id', (req, res) => {
  try {
    const id = Number(req.params.id);
    const user = req.session.user!;
    if (!Number.isFinite(id)) {
...
// DELETE /api/faq/:id
faqAdminRouter.delete('/:id', (req, res) => {
  try {
    const id = Number(req.params.id);
    const user = req.session.user!;
    if (!Number.isFinite(id)) {
